import ROOT as rt
from utils import SiPixelDetsUpdatedAfterFlippedChange_BPIX as pos
...
def write_pos_fl(histo, pos_dict):
    fw = open('data/fluence/fluka_l1.py','w')
...
def main(args):
    f = rt.TFile.Open("data/fluence/fluence_field_phase1_6500GeV.root")
    histo = f.Get("fluence_allpart")
    to_do = args.to_do
    if to_do == 'write':
        pos_dict = pos.name_pos_map
        write_pos_fl(histo, pos_dict)
    elif to_do == 'aver':
        from data.fluence import fluka_l1
