	   ...
	   #
		...
		# """
			...
			if count != 0:
				...
				while endtime2 < endtimeBetweenFills:
					...
					if (abs(Tdiff)>3. or (endtime2-begintime4) > datetime.timedelta(2,-1)) and temps > -300:
						...
						iLeak = -99.
						diLeak = -99.
						proFile.write("%d\t%4.2f\t%10d\t%4.2f\t%6.2f\t%6.2f\n"%(timeInterval,averTemp+zeroCelsius,0,averTemp+zeroCelsius,iLeak,diLeak))#,begintime3,endtime3,fillNum))
						temps = []
						begintime3=endtime3
			...
			# if False:
				...
				totLumi+=float(lumi)
				proFile.write("%d\t%4.2f\t%10d\t%4.2f\t%6.2f\t%6.2f\n"%(timeInterval,averTemp+zeroCelsius+corrTemp,int(my_rog.getAverageFluenceSum(th2f_fluence,float(lumi))/float(timeInterval)),trueTemp+zeroCelsius,iLeak,diLeak))#,beginSTBtime,endSTBtime,fillNum))
			else:
