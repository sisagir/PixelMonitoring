...
class ROG(object):
	...
	def getFluenceNearestSlice(self,fluence_th2f,aver_phi):
		...
		phi_eq = conversionFactor*aver_phi/(self.getAverageFluenceSum(fluence_th2f,1.))*fluence_th2f.GetBinContent(fluence_th2f.GetXaxis().FindBin(my_mod.position.R0-my_mod.size.dR/2.+my_mod.dr/2.), fluence_th2f.GetYaxis().FindBin(my_mod.position.Z0))*len(self.list_of_modules)
		return phi_eq
