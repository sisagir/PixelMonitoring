...
def _cms_draw(canvas):
        ...
        extraText2 = "Barrel Pixel, Phase-1, Layer 1"
        extraTextFont = 51
        lumiTextSize = 0.5
        lumiTextOffset = 0.0
        cmsTextSize = 0.5
        ...
        latex.SetTextSize(cmsTextSize*t*factor)
        latex.DrawLatex(l+0.028, 1-t-0.04, cmsText)
        latex.SetTextFont(extraTextFont)
        latex.SetTextSize(extraTextSize*t*factor)
        latex.DrawLatex(l+0.12, 1-t-0.04, extraText)
      ...
      #   latex.SetTextSize(extraTextSize*t*1.2)
        latex.SetTextFont(51)
        latex.SetTextSize(extraTextSize*t*factor)
        latex.DrawLatex(l+0.42, 1-t-0.04, extraText3)
        latex.SetTextFont(62)
        latex.SetTextSize(extraTextSize*t*factor)
        latex.DrawLatex(l+0.035, 1-t-0.45, extraText2)
...
def main():
 ...
 # month = "july"
 outfilename = {
    "april":"fl_vs_z_all_apr.pdf",
    "july":"fl_vs_z_all_jul.pdf"
 }
 
 th1d_fl_vs_z_lin = rt.TH1D("Fluence_vs_z_lin","Deviation between data and simulated fluence",4,0,25.7)
 ...
 c1 = rt.TCanvas("c1","c1",0,0,1600,1600)
 # print(vdepl_arr["april"])
 # print(fl)
 for month in ["april","july"]:
  for i in range(0,4):
   th1d_fl_vs_z_equiv.SetBinContent(i+1,(vdepl_arr[month][i]/fl[i])/(vdepl_arr[month][0]/fl[0]))
   th1d_fl_vs_z_equiv.SetBinError(i+1,(vdepl_arr[month][i]/fl[i])/(vdepl_arr[month][0]/fl[0])*
    math.sqrt(
    (vdepl_arr_err[month][i]/vdepl_arr[month][i])**2+
    (fl_error[i]/fl[i])**2+
    (vdepl_arr_err[month][0]/vdepl_arr[month][0])**2+
    (fl_error[0]/fl[0])**2
    )
   )
   if i == 0:
      th1d_fl_vs_z_equiv.SetBinError(i+1,0.)
   th1d_fl_vs_z_lin.SetBinContent(i+1, alpha_lin[month][i]/alpha_lin[month][0])
   th1d_fl_vs_z_lin.SetBinError(i+1, alpha_lin[month][i]/alpha_lin[month][0]*
    math.sqrt((alpha_lin_err[month][i]/alpha_lin[month][i])**2+(alpha_lin_err[month][0]/alpha_lin[month][0])**2))
   if i == 0:
      th1d_fl_vs_z_lin.SetBinError(i+1, 0.)
   th1d_fluka_vs_z.SetBinContent(i+1, fl[i]/fl[0])
   th1d_fluka_vs_z.SetBinError(i+1, fl[i]/fl[0]*math.sqrt(
    (fl_error[i]/fl[i])**2+
    (fl_error[0]/fl[0])**2
    )
   )
   ...
   th1d_v_vs_z.SetBinContent(i+1, vdepl_arr[month][i]/vdepl_arr[month][0])
   th1d_v_vs_z.SetBinError(i+1,vdepl_arr[month][i]/vdepl_arr[month][0]*math.sqrt(
    (vdepl_arr_err[month][i]/vdepl_arr[month][i])**2+
    (vdepl_arr_err[month][0]/vdepl_arr[month][0])**2
    )
   )
  ...
  th1d_fl_vs_z_lin.GetYaxis().SetTitle("Normalized units")
...
#   th1d_fl_vs_z_equiv.Draw("same p")
  ...
  leg = rt.TLegend(0.13,0.125,0.85,0.35)
  leg.SetBorderSize(0)
#   leg.AddEntry(th1d_fl_vs_z_lin, "(#Phi_{eq}^{real}(z)/#Phi_{eq}^{real}(0))/(#Phi_{eq}^{fluka}(z)/#Phi_{eq}^{fluka}(0))")
  leg.AddEntry(th1d_fl_vs_z_lin, "#alpha_{N}/#alpha_{1} - fraction of ratios of data and simulated fluence")
#   leg.AddEntry(th1d_fl_vs_z_equiv, "(V(z)/V(0))/(#Phi_{eq}^{fluka}(z)/#Phi_{eq}^{fluka}(0))")
  leg.AddEntry(th1d_fluka_vs_z, "#Phi_{eq}^{fluka}(z)/#Phi_{eq}^{fluka}(0) - ratio between simulated fluence")
  leg.AddEntry(th1d_v_vs_z, "V_{depl}(z)/V_{depl}(0) - ratio between depletion voltage")
  leg.Draw("same")
